    ...
    return {
        ...
        visitor: {
            CallExpression(path, state) {
                ...
                //3.计算泛型的类型 T => number W=>string
                const genericMap = new Map();
                const funcName = path.get('callee').node.name;
                ...
                //console.log(genericMap);//{T:number,W:string}
                //4.计算形参的类型
                ...
                paramTypes.forEach((type, index) => {
                    if (type !== argumentsTypes[index]) {
                        ...
                        errors.push(
                            path.get('init').buildCodeFrameError(`无法把${argumentsTypes[index]}赋值给${type}`, Error)
                        );
                    }
                });
